#!/usr/bin/env python3
"""Parametric visualization system for experiments_hydra results.
...
def create_histogram_grid(
    ...
    # Create title
    if y_axis == "none":
        title = f"Model Predictions vs Ground Truth\n{x_axis.replace('_', ' ').title()}\nBody Part: {body_part}"
    else:
...
def create_individual_trials_plot(
    ...
    title = f"Individual Trial Distributions\n{y_axis.replace('_', ' ').title()} (rows) × {x_axis.replace('_', ' ').title()} (columns)\nBody Part: {body_part}"
    if active_filters:
        filter_str = ", ".join(f"{k}={v}" for k, v in active_filters.items())
        title += f"\nFilters: {filter_str}"
...
def main():
    parser = argparse.ArgumentParser(
        description="Create parametric visualizations for experiments_hydra results"
    )
    parser.add_argument(
        "--type",
    ...
    parser.add_argument(
        "--body-part",
        default="wrist",
        help="Body part(s) to analyze. Use comma-separated values for multiple (e.g., 'wrist,forearm') (default: wrist)",
    )
    parser.add_argument(
        "--results-file",
        default=None,
    ...
    if len(body_parts_to_process) == 1:
        ...
        print(
            f"🎯 Body parts: {', '.join(body_parts_to_process)} ({len(body_parts_to_process)} total)"
        )
    ...
    if multi_value_filter:
        print(
            f"📊 Multi-value filter: {multi_value_filter_name} will create separate bars for each value"
        )
    ...
    try:
            ...
            for var, filter_values in filters.items():
                if filter_values:
                    ...
                    if len(filter_values) == 1:
                        ...
                        # Exception: don't filter by single body_part when it's the multi-value filter
                        if not (
        ...
        # Determine body_part for grouping (use first one if multi-value, or the single value)
        ...
        if args.type == "histogram":
            ...
            if multi_value_filter:
                # Handle multi-value filters by creating separate heatmaps for each filter value
                filter_values_order = (
                    filters.get(multi_value_filter)
                    if filters.get(multi_value_filter)
                    else list(set(key[2] for key in metrics.keys()))
                )
                ...
                if args.metric == "all":
                    ...
                    print(
                        f"🔥 Creating heatmaps for all {len(all_metrics)} metrics across {len(filter_values_order)} {multi_value_filter} values..."
                    )

                    for filter_value in filter_values_order:
                        print(f"  📊 Processing {multi_value_filter}: {filter_value}")
                    ...
                    print(
                        f"🔥 Creating {args.metric} heatmaps for {len(filter_values_order)} {multi_value_filter} values..."
                    )
        ...
        if args.type == "heatmap":
            if multi_value_filter:
                ...
                if args.metric == "all":
                    ...
                    print(
                        f"✅ {total_heatmaps} heatmap visualizations completed for all metrics across {len(filter_values_order)} {multi_value_filter} values!"
                    )
                else:
                    print(
                        f"✅ {len(filter_values_order)} heatmap visualizations completed for {args.metric} across {len(filter_values_order)} {multi_value_filter} values!"
                    )
            else:
                if args.metric == "all":
