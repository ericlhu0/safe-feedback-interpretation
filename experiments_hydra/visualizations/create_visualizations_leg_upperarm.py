#!/usr/bin/env python3
"""Parametric visualization system for experiments_hydra results - LEG using UPPER ARM labels.

This script creates histograms, heatmaps, and individual trial plots with
configurable X and Y axes based on experimental variables.

TEMPORARY FILE: This version evaluates LEG data using UPPER ARM labels.
"""
...
def create_histogram_grid(
    ...
    Args:
        stats: Statistics grouped by (x_value, y_value) or (x_value, y_value, filter_value)
    ...
    if has_filter_dimension:
        ...
        print(
            f"Multi-value filter detected: {len(filter_values)} filter values: {filter_values}"
        )
    else:
    ...
    # Create title
    if y_axis == "none":
        title = f"Model Predictions vs Ground Truth (LEG data with UPPER ARM labels)\n{x_axis.replace('_', ' ').title()}\nBody Part: {body_part}"
    else:
        title = f"Model Predictions vs Ground Truth (LEG data with UPPER ARM labels)\n{y_axis.replace('_', ' ').title()} (rows) × {x_axis.replace('_', ' ').title()} (columns)\nBody Part: {body_part}"
        ...
        for j, x_val in enumerate(x_values):
            ...
            if has_filter_dimension:
                ...
                if filter_data:
                    ...
                    for filter_idx, filter_val in enumerate(filter_values):
                        if filter_val in filter_data:
                            ...
                            for bar, prob in zip(pred_bars, model_probs):
                                if prob > 0.01:
                                    ax.text(
                    ...
                    # Draw ground truth lines for each filter value (body part), only if labels exist and not hidden
                    if not no_ground_truth:
                        ...
                        for filter_idx, filter_val in enumerate(filter_values):
                            if filter_val in filter_data:
                                ...
                                if label_probs is not None and n_label_trials > 0:
                                    # Calculate offset for this filter's ground truth lines
                                    offset = (
                                        filter_idx - (n_filters - 1) / 2
                                    ) * bar_width
                                    for k, prob in enumerate(label_probs):
                                        if prob > 0.001:
                                            # Position the ground truth line to align with the corresponding bar
                                            bar_center = k + offset
                                            ax.hlines(
                                                y=prob,
                        ...
                        # Add invisible line for ground truth legend only if any labels were drawn
                        if any_labels_drawn:
                            ax.hlines(
                                [],
                                [],
                                ...
                                linewidth=3,
                                alpha=0.8,
                                label="Ground Truth (Upper Arm)",
                            )

                    ax.set_title(
                        f"{y_axis}: {y_val}, {x_axis}: {x_val}\n(Total n={total_trials})",
                        fontsize=10,
                    )
                ...
                if key in stats:
                    ...
                    # Draw red horizontal lines for ground truth if labels exist and not disabled
                    if (
                        not no_ground_truth
    ...
    # Save plot
    filename = f"histogram_grid_{y_axis}_x_{x_axis}_{body_part}_LEG_UPPERARM.png"
...
def create_heatmap(
    ...
    title = f"{metric.replace('_', ' ').title()} (LEG data with UPPER ARM labels)\n{y_axis.replace('_', ' ').title()} (rows) × {x_axis.replace('_', ' ').title()} (columns)\nBody Part: {body_part}"
    if active_filters:
        filter_str = ", ".join(f"{k}={v}" for k, v in active_filters.items())
        title += f"\nFilters: {filter_str}"
    ...
    # Save plot
    filename = f"heatmap_{metric}_{y_axis}_x_{x_axis}_{body_part}_LEG_UPPERARM.png"
...
def create_individual_trials_plot(
    ...
    class_labels = ["Very Low", "Low", "Medium", "High", "Very High"]
    colors = plt.cm.Set3(np.linspace(0, 1, 12))  # Distinct colors for trials
    ...
    title = f"Individual Trial Distributions (LEG data with UPPER ARM labels)\n{y_axis.replace('_', ' ').title()} (rows) × {x_axis.replace('_', ' ').title()} (columns)\nBody Part: {body_part}"
    if active_filters:
        filter_str = ", ".join(f"{k}={v}" for k, v in active_filters.items())
        title += f"\nFilters: {filter_str}"
    ...
    # Save plot
    filename = f"individual_trials_{y_axis}_x_{x_axis}_{body_part}_LEG_UPPERARM.png"
...
# Custom data loading function that uses leg data but upper arm labels
def load_and_modify_results_data(results_file: str) -> list:
    """Load results and modify to use leg data with upper arm labels."""
    from data_utils import load_results_data

    results = load_results_data(results_file)

    # Modify results: use leg data but replace labels with upper_arm labels
    modified_results = []
    for result in results:
        if result.get("config_used", {}).get("body_part") == "leg":
            # Create a copy and modify labels
            modified_result = result.copy()
            labels = modified_result.get("labels", {})

            # Replace leg labels with upper_arm labels if available
            if "upper_arm" in labels:
                # Replace labels with upper_arm labels but keep the config as leg
                modified_labels = labels.copy()
                modified_labels["leg"] = labels["upper_arm"]
                modified_result["labels"] = modified_labels

            modified_results.append(modified_result)

    print(f"Modified {len(modified_results)} leg results to use upper arm labels")
    return modified_results


def main():
    parser = argparse.ArgumentParser(
        description="Create parametric visualizations for experiments_hydra results - LEG using UPPER ARM labels"
    )
    parser.add_argument(
        "--type",
    ...
    parser.add_argument(
        ...
        help="Variable for Y axis (default: verbal_intensity, use 'none' for flat layout)",
    )
    parser.add_argument(
        "--metric",
        choices=[
    ...
    parser.add_argument(
        "--results-file",
        default=None,
        help="Path to results file (default: ../results/results.jsonl relative to script)",
    )
    parser.add_argument(
        "--output-dir",
        default="outputs_leg_upperarm",
        help="Output directory for plots (default: outputs_leg_upperarm)",
    )

    # Filter arguments for non-plotted variables
    parser.add_argument(
        "--verbal-filter",
        help="Filter to specific verbal_intensity value(s). Use comma-separated values for multiple (e.g., 'high,mid')",
    )
    parser.add_argument(
        "--facial-filter",
        help="Filter to specific facial_intensity value(s). Use comma-separated values for multiple (e.g., 'high,mid')",
    )
    parser.add_argument(
        "--source-filter",
        help="Filter to specific source_specificity value(s). Use comma-separated values for multiple (e.g., 'very,region')",
    )
    parser.add_argument(
        "--modality-filter",
        help="Filter to specific face_modality value(s). Use comma-separated values for multiple (e.g., 'img,text')",
    )
    parser.add_argument(
        "--no-ground-truth",
        action="store_true",
    ...
    def parse_filter_values(filter_arg):
    ...
    filters = {
        ...
        "body_part": ["leg"],  # Force leg only
    }

    for var, filter_values in filters.items():
        if filter_values and (var == args.x_axis or var == args.y_axis):
            print(
                f"❌ Error: Cannot filter {var} because it's on the {var == args.x_axis and 'X' or 'Y'} axis"
            )
            sys.exit(1)

    print(f"🚀 Creating {args.type} visualization (LEG data with UPPER ARM labels)...")
    print(f"📊 Axes: {args.y_axis} (Y) × {args.x_axis} (X)")
    print(f"🎯 Body part: leg (using upper arm labels)")

    # Show active filters
    active_filters = {}
    for k, v in filters.items():
        if v and k != "body_part":  # Don't show body_part filter in title
            if len(v) > 1:
    ...
    try:
        results = load_and_modify_results_data(str(args.results_file))
        ...
        grouped_results = group_results_by_axes(
            filtered_results, args.x_axis, args.y_axis, "leg"
        )
        ...
        if args.type == "histogram":
            ...
            create_individual_trials_plot(
                stats, args.x_axis, args.y_axis, output_dir, "leg", active_filters
            )

        # Success message
        if args.type == "heatmap":
            if args.metric == "all":
                print(f"✅ Heatmap visualizations completed for all metrics!")
            else:
                print(f"✅ Heatmap visualization completed!")
        else:
