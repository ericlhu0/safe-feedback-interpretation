...
def group_results_by_axes_with_filter(
    ...
    y_axis: str,
    body_part: str = "wrist",
    multi_value_filter: str = None,
    multi_filter_values: List[str] = None,
    ...
    Args:
        ...
        y_axis: Variable name for Y axis (e.g., "verbal_intensity")
        body_part: Body part to analyze (default: "wrist")
        multi_value_filter: Variable name for multi-value filter (e.g., "verbal_intensity")
    ...
    print(
        f"Grouped results by {y_axis} (Y) × {x_axis} (X) × {multi_value_filter} (Filter)"
    )
    print(f"Found combinations: {list(grouped.keys())}")
    print(f"Total entries per combination: {[(k, len(v)) for k, v in grouped.items()]}")
...
def calculate_average_distributions(
    ...
    Args:
        grouped_results: Results grouped by (x_value, y_value) or (x_value, y_value, filter_value)
    ...
    for key_tuple, results in grouped_results.items():
        ...
        if model_distributions:
            ...
            if label_distributions:
                ...
                # Mark label fields as None-equivalents to signal plotting to skip markers
...
def print_data_summary(
    ...
    if grouped_results:
        ...
        if has_filter_dimension:
            ...
            print(
                f"\nGrid layout with multi-value filter ({len(y_values)}×{len(x_values)}×{len(filter_values)}):"
            )
