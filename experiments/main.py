"""Main experiment runner for safe feedback interpretation experiments."""
...
def load_existing_results(results_file: Path) -> set:
    ...
    duplicates."""
    existing_experiments: set[tuple] = set()
            ...
            facial_options.append(
                {
                    "modality": "image",
                    "description": (
                        "Please examine the provided facial expression image "
                        "to assess comfort level."
                    ),
...
def build_model_input(
    ...
    comfort_threshold = input_context["current_comfort_threshold"]
    current_threshold = comfort_threshold["current_comfort_threshold"]
    joint_range = comfort_threshold["current_comfortable_joint_range_deg"]
    ...
    context_str = textwrap.dedent(
        ...
        Comfort threshold: {current_threshold}
        Comfortable joint range (deg): {joint_range}
        Received feedback:
...
def main(cfg: DictConfig) -> None:
    ...
    # Load experiment configs from experiments directory
    ...
    try:
        ...
        print(
            f"  verbal_intensity={verbal_intensity}, "
            f"facial_intensity={facial_intensity}, "
            f"source_specificity={source_specificity}"
        )
        ...
        try:
            with open(verbal_data_path, "r", encoding="utf-8") as f:
        ...
        for i, scenario in enumerate(filtered_scenarios):
            ...
            scenario_name = (
                f"{verbal_intensity}_{facial_intensity}_"
                f"{source_specificity}_{body_part}"
            )
            print(
                f"\n--- Processing scenario {i+1}/{len(filtered_scenarios)}: "
                f"{scenario_name} ---"
            )
            ...
            for j, feedback_combo in enumerate(feedback_combinations):
                print(
                    ...
                    f"Combination {j+1}/{len(feedback_combinations)} ---"
                )
                ...
                if experiment_id in existing_experiments:
                    print(
                        f"Skipping {scenario_name} combination "
                        f"{j+1}/{len(feedback_combinations)} - "
                        f"already exists in results"
                    )
                    continue
                ...
                # Prepare data to save with actual config values,
                # not from matched scenario
                ...
                try:
                    ...
                    print(
                        f"Saved combination {j+1}/{len(feedback_combinations)} "
                        f"to results file"
                    )
                except Exception as e:
            ...
            print(
                f"\nCompleted scenario {scenario_name} with "
                f"{len(feedback_combinations)} combinations"
            )
...
if __name__ == "__main__":
    main()  # pylint: disable=no-value-for-parameter  # hydra injects config
